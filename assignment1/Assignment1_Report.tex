...
\title{\Large\bfseries Solving the K-Armed Bandit Problem Using\\Reinforcement Learning}
\author{Assignment 1 Report \\ Gaurav Prakash \\ MSAI 635, Reinforcement Learning}
\date{}
...
The k-armed bandit is a single-state reinforcement learning problem used to study the
exploration-exploitation tradeoff in isolation. At each time step $t$, an agent selects one
of $k = 10$ arms (actions). Each arm $a$ has a true, fixed action value $q_*(a)$ drawn once
at the start of a run from $\mathcal{N}(0, 1)$. Selecting arm $a$ returns a stochastic reward
$R \sim \mathcal{N}(q_*(a), 1)$. The agent does not know $q_*(a)$ and must estimate it online
from observed rewards while trying to maximize cumulative reward over 1000 steps. Because the
reward is noisy and the true values are unknown, the agent faces a fundamental decision at
every step: exploit the action that currently looks best, or explore other actions to improve
its estimates and potentially find a better one. Performance is measured with two metrics,
...
sampling noise in the first few rewards rather than by the true action values. Exploration
($\varepsilon > 0$) forces the agent to keep sampling arms it has not settled on, which
corrects these early, noisy estimates over time and lets the true optimal arm's higher
average reward eventually stand out from the noise. This directly improves long-run learning
performance: both $\varepsilon$-greedy agents keep raising their \% optimal-action and
average-reward curves throughout the full 1000 steps, whereas an agent with no exploration
stops learning about arms it has abandoned. The cost of exploration is short-term: every
exploratory step has an expected reward equal to the average of all arms rather than the best
one, so more exploration trades away some immediate reward for better long-run action-value
estimates.
...
is higher. If the actual best arm was undersampled or unlucky early, its estimate never gets
the chance to catch up, and the agent locks in on a suboptimal action for the rest of the run.
...
$\varepsilon$ would be almost pure exploration and would rarely capitalize on what it learns,
capping average reward well below the optimal arm's true value even after the optimal arm is
correctly identified. A well-chosen, moderate $\varepsilon$ spends enough steps exploring to
reliably identify the best arm, while still spending most steps exploiting it once found ---
this is why $\varepsilon$-greedy with a small, nonzero $\varepsilon$ strictly dominates pure
greedy action selection in this experiment, and why the specific value of $\varepsilon$
matters (Section~5).
...
1000 time steps per run, averaged over 200 independent runs, each with a freshly sampled set
of true action values $q_*(a) \sim \mathcal{N}(0, 1)$ and independent reward noise.
...
in the first \textasciitilde50 steps but then plateaus almost completely: \% optimal action
flattens at \textasciitilde38\% and average reward stalls around 1.0--1.1 for the rest of the
...
at step 1000 (\textasciitilde54.5\% optimal action, average reward \textasciitilde1.32)
...
\textasciitilde85\% optimal action and the highest average reward
(\textasciitilde1.36--1.55 in the later steps) within the 1000-step budget.

\subsection{Why greedy performs poorly here}
The flat blue line in Figure~1 is a direct, visible symptom of the greedy method's core flaw
described in Section~2: once it exploits an early-favored arm, it never revisits its estimates
for the other nine arms, so it cannot recover if that arm is not actually optimal. Its 38\%
optimal-action rate implies that in the majority of the 200 runs, the greedy agent locked onto
the wrong arm and stayed there for the entire episode --- a purely structural limitation, not
a result of insufficient steps.
...
needs a much longer horizon to pay off. In practice this suggests that a fixed $\varepsilon$
is itself a compromise --- a decaying $\varepsilon$ (high early for fast identification, low
later for reward maximization), optimistic initial values, or upper-confidence-bound (UCB)
action selection would let an agent capture the fast convergence benefit of high exploration
early on without permanently paying its long-run cost.
